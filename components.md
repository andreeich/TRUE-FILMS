...
const defaultOptions = {
  ...
  maxView: 3,
  minDate: null,
  nextArrow: '<svg width="16" height="8" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M16 3.556L0 7.11V0l16 3.556z" fill="#7A9F8F"/></svg>',
  orientation: 'auto',
  pickLevel: 0,
  prevArrow: '<svg width="16" height="8" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M0 3.556L16 0v7.111L0 3.556z" fill="#7A9F8F"/></svg>',
  showDaysOfWeek: true,
  showOnClick: true,
  showOnFocus: true,
  startView: 0,
...
const pickerTemplate = (0,utils/* optimizeTemplateHTML */.zh)(`<div class="hidden datepicker">
  <div class="inline-block bg-center bg-no-repeat bg-cover pt-[17px] pb-[37px] px-[8px] w-[336px] h-[307px] bg-datepicker datepicker-picker">
    <div class="datepicker-header">
      <div class="px-2 py-3 font-semibold text-center bg-white datepicker-title"></div>
      <div class="flex justify-between mb-2 datepicker-controls">
        <button type="button" class="focus:outline-none prev-btn"></button>
        <button type="button" class="font-bold text-[26px] text-primary-300 focus:outline-none view-switch"></button>
        <button type="button" class="focus:outline-none next-btn"></button>
      </div>
    </div>
    <div class="px-[36px] datepicker-main"></div>
    <div class="datepicker-footer">
      <div class="flex mt-2 space-x-2 datepicker-controls">
        <button type="button" class="%buttonClass% today-btn text-white bg-primary-200 dark:bg-blue-600 hover:bg-blue-800 dark:hover:bg-primary-200 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2 text-center w-1/2"></button>
        <button type="button" class="%buttonClass% clear-btn text-gray-900 bg-white border border-gray-300 dark:border-gray-600 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2 text-center w-1/2"></button>
      </div>
    </div>
  </div>
</div>`);
...
const daysTemplate = (0,utils/* optimizeTemplateHTML */.zh)(`<div class="days">
  <div class="grid grid-cols-7 mb-1 place-items-center days-of-week">${(0,utils/* createTagRepeat */.em)('span', 7, {class: 'dow block flex-1 leading-9 border-0 rounded-lg cursor-default text-center text-gray-900 font-semibold text-sm'})}</div>
  <div class="grid grid-cols-7 place-items-center w-[248px] h-[162px] datepicker-grid">${(0,utils/* createTagRepeat */.em)('span', 42 , {class: 'block flex-1 leading-9 border-0 rounded-lg cursor-default text-center text-gray-900 font-semibold text-sm h-6 leading-6 text-sm font-medium text-gray-500 dark:text-gray-400'})}</div>
</div>`);
...
class View {
  constructor(picker, config) {
    Object.assign(this, config, {
      picker,
      element: parseHTML(`<div class="flex datepicker-view"></div>`).firstChild,
      selected: [],
    });
...
class DaysView extends View {
  ...
  setOptions(options) {
    ...
    if (updateDOW) {
      Array.from(this.dow.children).forEach((el, index) => {
        ...
        el.className = this.daysOfWeekDisabled.includes(dow) ? 'dow disabled text-center h-[22px] w-[22px] leading-[22px] text-[11px] text-primary-200 cursor-not-allowed' : 'dow text-center h-[22px] w-[22px] leading-[22px] text-[11px] text-primary-200';
      });
    }
  }
  ...
  render() {
    ...
    Array.from(this.grid.children).forEach((el, index) => {
      ...
      el.className = `datepicker-cell hover:bg-primary-100 block flex-1 leading-[22px] w-[22px] h-[22px] border-0 rounded-[4px] cursor-pointer text-center text-primary-300 font-bold text-[11px] ${this.cellClass}`;
      ...
      if (this.selected.includes(current)) {
        classList.add('selected', 'bg-primary-200', 'text-white');
        classList.remove('text-primary-300','hover:bg-primary-100');
      }
  ...
  refresh() {
    ...
    this.grid
      ...
      .forEach((el) => {
        el.classList.remove('range', 'range-start', 'range-end', 'selected', 'bg-primary-200', 'text-white', 'focused');
        el.classList.add('text-primary-300', 'hover:bg-primary-100');
      });
...
class MonthsView extends View {
  ...
  init(options, onConstruction = true) {
    if (onConstruction) {
      ...
      this.element.classList.add('months', 'datepicker-grid', 'w-[248px]','h-[162px]', 'grid', 'grid-cols-4', 'place-items-center');
  ...
  render() {
    ...
    Array.from(this.grid.children).forEach((el, index) => {
      ...
      el.className = `datepicker-cell hover:bg-primary-100 block flex-1 leading-[22px] border-0 rounded-[4px] cursor-pointer text-center text-primary-300 font-bold text-[11px] h-[22px] w-[32px] ${this.cellClass}`;
      if (this.isMinView) {
...
class YearsView extends View {
  ...
  init(options, onConstruction = true) {
    if (onConstruction) {
      ...
      this.grid = this.element;
      this.element.classList.add(this.name, 'datepicker-grid', 'w-[248px]', 'h-[162px]', 'grid', 'grid-cols-4', 'place-items-center');
...
class Picker {
  ...
  setViewSwitchLabel(labelText) {
    this.controls.viewSwitch.innerHTML = labelText;
  }
